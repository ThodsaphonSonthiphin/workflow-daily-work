...
```mermaid
flowchart TD
    Q{how much line overlap confirms<br/>a copy is ours?} -->|chosen| A["0.70 - forced by arithmetic; the design's
    own 'unrelated' fixture scores 0.667 and
    must grade UNRELATED"]
    Q -->|rejected| B["0.60, as the design stated - but at 0.60
    that same fixture grades STALE, so the
    design contradicted its own test and
    could not be implemented as written"]
```

This was not a judgement call refined into a number; the number was already determined
and the design had it wrong. The spec's own worked example of a same-named file from a
different lineage shares two of three non-blank lines with the source - 0.6667 - and the
spec asserts it must grade `UNRELATED`. At the stated 0.60 it grades `STALE`. One of the

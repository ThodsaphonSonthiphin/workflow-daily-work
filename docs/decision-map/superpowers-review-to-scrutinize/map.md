...
- [Distribution - what makes sure a colleague has superpowers installed before the copies need it?](tickets/override-distribution.md) — The skillOverrides question is VOID - re-scoped to the prerequisite that survives: the host SessionStart hook checks that superpowers is installed, and the README gains the line.

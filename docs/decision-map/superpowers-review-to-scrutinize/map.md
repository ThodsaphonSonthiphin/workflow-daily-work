...
- [Distribution - how do the six skillOverrides entries reach a colleague's machine?](tickets/override-distribution.md) — Re-scoped off the void skillOverrides premise: four manual steps cannot be shipped, and a read-only dev-workflows setup-check reports all four - the ado-backlog/github-backlog pattern.

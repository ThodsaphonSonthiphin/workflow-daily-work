...
- **The generated skills tree is committed:** after any edit under `plugins/*/skills/` OR to a file the generator copies into a skill (`plugins/decision-map/scripts/*.py`, `plugins/decision-map/references/data-contracts.md`), run `python3 scripts/generate_skills_tree.py` from the repo root and commit the resulting `skills/…` changes; `python3 scripts/check_skills_tree.py --repo .` must exit 0. Never hand-edit `skills/`.

...
> **SUPERSEDED IN PART — implemented 2026-08-03.**
>
> | This spec says | What is now true |
> |---|---|
> | The renderer is `map_core.render_position_diagram(key, parents, children)` (§"A new generated region") | Shipped as `map_core.position_diagram_region(key, parents, children)`. It returns the whole marker-delimited region, not a bare diagram, and the name says so; `map_core.set_graph_region(body, region)` is its replace-or-insert companion. |
> | "When it is written": `chart --real` and `block` are the subcommands that change an edge | Still true of the subcommands that ADD an edge. `chart --force` **removes** edges, which stales the diagram at both ends of each one; both backends now re-render the OVERWRITE'd ticket and any blocker that lost a child. See ADR 0064. |
